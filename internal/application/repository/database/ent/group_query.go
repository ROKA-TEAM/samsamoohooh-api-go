...
func (gq *GroupQuery) QueryPosts() *PostQuery {
	query := (&PostClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryTasks() *TaskQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, group.TasksTable, group.TasksColumn),
		)
		fromU = sqlgraph.SetNeighbors(gq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (gq *GroupQuery) WithPosts(opts ...func(*PostQuery)) *GroupQuery {
	query := (&PostClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withPosts; query != nil {
		if err := gq.loadPosts(ctx, query, nodes,
			func(n *Group) { n.Edges.Posts = []*Post{} },
			func(n *Group, e *Post) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GroupQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Group, init func(*Group), assign func(*Group, *Post)) error {
	...
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.PostsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_posts
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_posts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (gq *GroupQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Group, init func(*Group), assign func(*Group, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.TasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.group_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
