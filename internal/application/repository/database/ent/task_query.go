...
func (tq *TaskQuery) QueryTopics() *TopicQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, task.TopicsTable, task.TopicsColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TaskQuery) WithTopics(opts ...func(*TopicQuery)) *TaskQuery {
	query := (&TopicClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withGroup; query != nil {
		if err := tq.loadGroup(ctx, query, nodes, nil,
			func(n *Task, e *Group) { n.Edges.Group = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTopics; query != nil {
		if err := tq.loadTopics(ctx, query, nodes,
			func(n *Task) { n.Edges.Topics = []*Topic{} },
			func(n *Task, e *Topic) { n.Edges.Topics = append(n.Edges.Topics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Task, init func(*Task), assign func(*Task, *Group)) error {
	...
	for i := range nodes {
		if nodes[i].group_tasks == nil {
			continue
		}
		fk := *nodes[i].group_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadTopics(ctx context.Context, query *TopicQuery, nodes []*Task, init func(*Task), assign func(*Task, *Topic)) error {
	...
	query.Where(predicate.Topic(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TopicsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.task_topics
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_topics" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_topics" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
