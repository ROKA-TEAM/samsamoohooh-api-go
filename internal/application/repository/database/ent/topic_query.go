...
func (tq *TopicQuery) QueryTask() *TaskQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, topic.TaskTable, topic.TaskColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TopicQuery) WithTask(opts ...func(*TaskQuery)) *TopicQuery {
	query := (&TaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if tq.withUser != nil || tq.withTask != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, topic.ForeignKeys...)
	}
	...
	if query := tq.withTask; query != nil {
		if err := tq.loadTask(ctx, query, nodes, nil,
			func(n *Topic, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_topics == nil {
			continue
		}
		fk := *nodes[i].user_topics
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_topics" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TopicQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Task)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Topic)
	for i := range nodes {
		if nodes[i].task_topics == nil {
			continue
		}
		fk := *nodes[i].task_topics
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_topics" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
