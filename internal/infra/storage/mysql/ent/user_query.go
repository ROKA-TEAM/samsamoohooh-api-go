...
func (uq *UserQuery) loadTopics(ctx context.Context, query *TopicQuery, nodes []*User, init func(*User), assign func(*User, *Topic)) error {
	...
	for _, n := range neighbors {
		fk := n.user_topics
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_topics" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_topics" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
