...
import (
	...
	"samsamoohooh-go-api/internal/infra/storage/mysql/ent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Comment, c.Group, c.Post, c.Task, c.Topic, c.User,
	} {
		n.Use(hooks...)
	}
}
...
// QueryTopics queries the topics edge of a Task.
func (c *TaskClient) QueryTopics(t *Task) *TopicQuery {
...
// QueryTask queries the task edge of a Topic.
func (c *TopicClient) QueryTask(t *Topic) *TaskQuery {
	query := (&TaskClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Comment, Group, Post, Task, Topic, User []ent.Hook
	}
	inters struct {
		Comment, Group, Post, Task, Topic, User []ent.Interceptor
	}
)
