...
func (pq *PostQuery) QueryGroup() *GroupQuery {
...
func (pq *PostQuery) WithGroup(opts ...func(*GroupQuery)) *PostQuery {
	query := (&GroupClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if pq.withUser != nil || pq.withGroup != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withGroup; query != nil {
		if err := pq.loadGroup(ctx, query, nodes, nil,
			func(n *Post, e *Group) { n.Edges.Group = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*Post, init func(*Post), assign func(*Post, *Group)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Post)
	for i := range nodes {
		if nodes[i].group_posts == nil {
			continue
		}
		fk := *nodes[i].group_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
